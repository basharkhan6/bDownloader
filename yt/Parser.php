<?php
  namespace YouTube;

  class Parser {
    private $itag_info = array(
      5 => "FLV 400x240",
      6 => "FLV 450x240",
      13 => "3GP Mobile",
      17 => "3GP 144p",
      18 => "MP4 360p",
      22 => "MP4 720p (HD)",
      34 => "FLV 360p",
      35 => "FLV 480p",
      36 => "3GP 240p",
      37 => "MP4 1080",
      38 => "MP4 3072p",
      43 => "WebM 360p",
      44 => "WebM 480p",
      45 => "WebM 720p",
      46 => "WebM 1080p",
      59 => "MP4 480p",
      78 => "MP4 480p",
      82 => "MP4 360p 3D",
      83 => "MP4 480p 3D",
      84 => "MP4 720p 3D",
      85 => "MP4 1080p 3D",
      91 => "MP4 144p",
      92 => "MP4 240p HLS",
      93 => "MP4 360p HLS",
      94 => "MP4 480p HLS",
      95 => "MP4 720p HLS",
      96 => "MP4 1080p HLS",
      100 => "WebM 360p 3D",
      101 => "WebM 480p 3D",
      102 => "WebM 720p 3D",
      120 => "WebM 720p 3D",
      127 => "TS Dash Audio 96kbps",
      128 => "TS Dash Audio 128kbps"
    );

    public function parseItagInfo($itag) {
      if (isset($this->itag_detailed[$itag])) {
        return $this->itag_detailed[$itag];
      }

      return 'Unknown';
      // return isset($this->itag_info[$itag]) ? $this->itag_info[$itag] : 'Unknown';
    }

    private $itag_detailed = array(
      5 => 'flv, 240p, video/audio',
      6 => 'flv, 270p, video/audio',
      13 => '3gp, video/audio',
      17 => '3gp, 144p, video/audio',
      18 => 'mp4, 360p, video/audio',
      22 => 'mp4, 720p, video/audio',
      34 => 'flv, 360p, video/audio',
      35 => 'flv, 480p, video/audio',
      36 => '3gp, video/audio',
      37 => 'mp4, 1080p, video/audio',
      38 => 'mp4, 3072p, video/audio',
      43 => 'webm, 360p, video/audio',
      44 => 'webm, 480p, video/audio',
      45 => 'webm, 720p, video/audio',
      46 => 'webm, 1080p, video/audio',
      59 => 'mp4, 480p, video/audio',
      78 => 'mp4, 480p, video/audio',
      82 => 'mp4, 360p, video/audio',
      83 => 'mp4, 480p, video/audio',
      84 => 'mp4, 720p, video/audio',
      85 => 'mp4, 1080p, video/audio',
      100 => 'webm, 360p, video/audio',
      101 => 'webm, 480p, video/audio',
      102 => 'webm, 720p, video/audio',
      91 => 'mp4, 144p, video/audio',
      92 => 'mp4, 240p, video/audio',
      93 => 'mp4, 360p, video/audio',
      94 => 'mp4, 480p, video/audio',
      95 => 'mp4, 720p, video/audio',
      96 => 'mp4, 1080p, video/audio',
      132 => 'mp4, 240p, video/audio',
      151 => 'mp4, 72p, video/audio',
      133 => 'mp4, 240p, video only',
      134 => 'mp4, 360p, video only',
      135 => 'mp4, 480p, video only',
      136 => 'mp4, 720p, video only',
      137 => 'mp4, 1080p, video only',
      138 => 'mp4, video only',
      160 => 'mp4, 144p, video only',
      212 => 'mp4, 480p, video only',
      264 => 'mp4, 1440p, video only',
      298 => 'mp4, 720p, video only',
      299 => 'mp4, 1080p, video only',
      266 => 'mp4, 2160p, video only',
      139 => 'm4a, audio only',
      140 => 'm4a, audio only',
      141 => 'm4a, audio only',
      256 => 'm4a, audio only',
      258 => 'm4a, audio only',
      325 => 'm4a, audio only',
      328 => 'm4a, audio only',
      167 => 'webm, 360p, video only',
      168 => 'webm, 480p, video only',
      169 => 'webm, 720p, video only',
      170 => 'webm, 1080p, video only',
      218 => 'webm, 480p, video only',
      219 => 'webm, 480p, video only',
      278 => 'webm, 144p, video only',
      242 => 'webm, 240p, video only',
      243 => 'webm, 360p, video only',
      244 => 'webm, 480p, video only',
      245 => 'webm, 480p, video only',
      246 => 'webm, 480p, video only',
      247 => 'webm, 720p, video only',
      248 => 'webm, 1080p, video only',
      271 => 'webm, 1440p, video only',
      272 => 'webm, 2160p, video only',
      302 => 'webm, 720p, video only',
      303 => 'webm, 1080p, video only',
      308 => 'webm, 1440p, video only',
      313 => 'webm, 2160p, video only',
      315 => 'webm, 2160p, video only',
      171 => 'webm, audio only',
      172 => 'webm, audio only',
      249 => 'webm, audio only',
      250 => 'webm, audio only',
      251 => 'webm, audio only',
    );
  }
?>
